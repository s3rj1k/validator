...
// nolint: gosec
func Test_StringSymlinkTargetIsNotPath(t *testing.T) {
	...
	v := &StringSymlinkTargetIsNotPath{Name: "Name", Field: "/tmp/not_a_symlink"}
	...
	v = &StringSymlinkTargetIsNotPath{Name: "Name", Field: "/tmp/test_symlink"}
	e = validator.NewErrors()
	v.Validate(e)
	r.Equal(1, e.Count()) // symlink to existing folder is err
	r.Equal([]string{StringSymlinkTargetIsNotPathError(v)}, e.Get("Name"))

...
// nolint: gosec
func Test_StringIsPathWithSetGID(t *testing.T) {
	r := require.New(t)
	...
	for index, test := range tests {
		v := &StringIsPathWithSetGID{Name: "FileModes", Field: test.field}
		...
		if !test.valid {
			r.Equalf([]string{StringIsPathWithSetGIDError(v)}, e.Get(v.Name), "tc %d", index)
		}
	}
}

...
func Test_StringIsSymlinkAndTargetIsPathDive(t *testing.T) {
	r := require.New(t)
	...
	var tests = []struct {
		...
		{[]string{notSymlink, symlinkTargetIsFile, symlinkTargetIsDir, notExists, symlinkTargetNotExist}, false, []int{4}},
	...
	for index, test := range tests {
		v := &StringSliceDive{
			Validator: &StringIsSymlinkAndTargetIsPath{Name: "SymlinkTargetIsDir"},
			Field:     test.field,
		}

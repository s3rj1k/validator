...
// nolint: gosec
func Test_StringIsPathWithoutSetUID(t *testing.T) {
	r := require.New(t)
	...
	for index, test := range tests {
		v := &StringIsPathWithoutSetUID{Name: "FileModes", Field: test.field}
		...
		if !test.valid {
			r.Equalf([]string{StringIsPathWithoutSetUIDError(v)}, e.Get(v.Name), "tc %d", index)
		}
	}
}

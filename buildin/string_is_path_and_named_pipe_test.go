...
// nolint: gosec
func Test_StringIsPathAndNamedPipe(t *testing.T) {
	r := require.New(t)
	...
	for index, test := range tests {
		v := &StringIsPathAndNamedPipe{Name: "FileModes", Field: test.field}
		...
		if !test.valid {
			r.Equalf([]string{StringIsPathAndNamedPipeError(v)}, e.Get(v.Name), "tc %d", index)
		}
	}
}
